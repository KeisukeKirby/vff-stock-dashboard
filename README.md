...
| 入荷予定 | 仕入先の梱包明細(Packing List .xls)の SKU 別足数。複数便は合算 |
| 在庫月数 | (在庫 + 入荷予定)÷ 今年の月平均 |
| 欠品中 | 在庫 + 入荷予定 が 0 かつ 今年の販売あり |
| 要発注 | 在庫月数(入荷予定含む)< リードタイム(既定 3ヶ月、画面で 2〜4 を選択) |
| 適正 | リードタイム 〜 12ヶ月 |
| 在庫過剰 | 在庫月数(入荷予定含む)> 12ヶ月 |
| 動きなし | 今年の販売 0 で在庫あり |
| 入荷待ち | 在庫 0・今年の販売 0 で入荷予定あり(新色/新モデル) |
